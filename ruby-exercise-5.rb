...
snowy_wordlength = snowy_words.sort_by{|line| line.length}
puts "#{snowy_wordlength.last}"

#Here, find the words that are in Snowy but not in the dictionary
puts
puts "Names in Snowy are:"
dictionary_words = dictionary.map{|word| word.downcase}
puts (snowy_words - dictionary_words).uniq.join ', '
#Look up the documentation for Set to see how to test if a word is in it or not

puts
puts "Names in both Snowy and Clancy:"
puts ((snowy_words + clancy_words) - dictionary_words).uniq.join ','
